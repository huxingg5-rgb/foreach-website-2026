...
function patchClient(content) {
  ...
  if (
    ...
    content.includes("isFittingProductOffline(") &&
  ...
  if (!result.includes(importPath)) {
    ...
    parsed = sourceFile(clientPath, result);
  }
  ...
  if (!arrayNode) {
    throw new Error(
      "没有找到 selectionProducts 数组。"
    );
  }

  const start = arrayNode.getStart(parsed);
  const end = arrayNode.getEnd();
  const arrayText = result.slice(start, end);
  const replacement =
