...
function buildLocalizedNarrative(
  ...
  const { model, series, tubeOd, thread, material, sealing, pressure } = values;

  if (locale === "es") {
    return `${model} es un ${series} para tubos rígidos con un diámetro exterior de ${tubeOd}. Incorpora una rosca ${thread}, un cuerpo de ${material} y ${sealing}.${pressure ? ` Su presión nominal es de ${pressure}.` : ""} Está diseñado para conexiones de microfluidos, equipos de diagnóstico in vitro (IVD), instrumentos analíticos y sistemas de automatización de laboratorio.`;
  }

  if (locale === "fr") {
    return `${model} est un ${series} destiné aux tubes rigides de diamètre extérieur ${tubeOd}. Il associe un filetage ${thread}, un corps en ${material} et une ${sealing}.${pressure ? ` Sa pression nominale est de ${pressure}.` : ""} Il convient aux circuits microfluidiques, aux équipements de diagnostic in vitro (IVD), aux instruments d’analyse et aux systèmes d’automatisation de laboratoire.`;
  }

  if (locale === "ko") {
    return `${model} 모델은 외경 ${tubeOd}의 경질 튜브용 ${series}입니다. ${thread} 나사, ${material} 본체 및 ${sealing} 구조를 적용했습니다.${pressure ? ` 정격 압력은 ${pressure}입니다.` : ""} 미세유체 연결, 체외진단(IVD) 장비, 분석기기 및 실험실 자동화 시스템에 적합합니다.`;
  }

  return `${model} — ${series} для жёстких трубок с наружным диаметром ${tubeOd}. Модель оснащена резьбой ${thread} и корпусом из ${material}; способ уплотнения — ${sealing}.${pressure ? ` Номинальное давление — ${pressure}.` : ""} Подходит для микрофлюидных трактов, оборудования IVD, аналитических приборов и систем лабораторной автоматизации.`;
}
...
function buildFaqs(
  ...
  if (locale === "es") {
    return [
      { question: `¿Para qué diámetro exterior de tubo está diseñado ${model}?`, answer: `${model} es adecuado para tubos rígidos con un diámetro exterior de ${tubeOd}. Antes de seleccionar el modelo, confirme también la tolerancia del tubo y la preparación de su extremo.` },
      { question: `¿Qué rosca utiliza ${model}?`, answer: `Este modelo utiliza una rosca ${thread}. Antes del montaje, compruebe que la rosca del puerto y la estructura de sellado sean compatibles.` },
      { question: `¿Cómo se verifica la compatibilidad química del ${material}?`, answer: `Compruebe la compatibilidad del ${material} teniendo en cuenta el fluido, la concentración, la temperatura, el tiempo de contacto y el método de limpieza.` },
      { question: `¿Qué debe revisarse al instalar este ${series}?`, answer: "Revise el extremo del tubo, la rosca, el elemento de sellado y la superficie del puerto. Todos deben estar limpios e intactos; evite un apriete excesivo." },
      { question: `¿Se puede solicitar un plano 2D de ${model}?`, answer: "Sí. Añada el modelo a la lista e indique que necesita el plano; el equipo de ingeniería confirmará la versión aplicable." },
    ];
  }

  if (locale === "fr") {
    return [
      { question: `Pour quel diamètre extérieur de tube le modèle ${model} est-il prévu ?`, answer: `${model} convient aux tubes rigides de diamètre extérieur ${tubeOd}. Avant la sélection, vérifiez également la tolérance du tube et la préparation de son extrémité.` },
      { question: `Quel filetage le modèle ${model} utilise-t-il ?`, answer: `Ce modèle utilise un filetage ${thread}. Avant le montage, vérifiez la compatibilité du filetage du port et de la structure d’étanchéité.` },
      { question: `Comment vérifier la compatibilité chimique du ${material} ?`, answer: `Vérifiez la compatibilité du ${material} selon le fluide, sa concentration, la température, la durée de contact et la méthode de nettoyage.` },
      { question: `Quels points contrôler lors du montage de ce ${series} ?`, answer: "Contrôlez l’extrémité du tube, le filetage, l’élément d’étanchéité et la face du port. Ils doivent être propres et intacts ; évitez tout serrage excessif." },
      { question: `Peut-on demander un plan 2D pour ${model} ?`, answer: "Oui. Ajoutez le modèle à votre liste et indiquez le besoin de plan ; l’équipe d’ingénierie confirmera la version applicable." },
    ];
  }

  if (locale === "ko") {
    return [
      { question: `${model}은 어떤 튜브 외경에 적합합니까?`, answer: `${model}은 외경 ${tubeOd}의 경질 튜브에 적합합니다. 선정 전 튜브 외경 공차와 단면 가공 상태도 확인해야 합니다.` },
      { question: `${model}에 적용된 나사 규격은 무엇입니까?`, answer: `이 모델은 ${thread} 나사를 사용합니다. 설치 전 장비 포트의 나사와 밀봉 구조가 호환되는지 확인하십시오.` },
      { question: `${material}의 유체 적합성은 어떻게 확인합니까?`, answer: `유체 종류, 농도, 온도, 접촉 시간 및 세척 방법을 기준으로 ${material}의 적합성을 확인하십시오.` },
      { question: `${series} 설치 시 무엇을 점검해야 합니까?`, answer: "튜브 단면, 나사, 밀봉 부품 및 포트 접촉면이 깨끗하고 손상되지 않았는지 확인하고 과도한 조임을 피하십시오." },
      { question: `${model}의 2D 도면을 요청할 수 있습니까?`, answer: "가능합니다. 모델을 목록에 추가하고 도면 필요 여부를 표시하면 엔지니어가 적용 가능한 버전을 확인합니다." },
    ];
  }

  return [
    { question: `Для какого наружного диаметра трубки предназначена модель ${model}?`, answer: `${model} подходит для жёстких трубок с наружным диаметром ${tubeOd}. Перед выбором также проверьте допуск наружного диаметра и качество обработки торца трубки.` },
    { question: `Какая резьба используется в модели ${model}?`, answer: `В этой модели используется резьба ${thread}. Перед монтажом убедитесь, что резьба порта и конструкция уплотнения совместимы.` },
    { question: `Как проверить химическую совместимость материала ${material}?`, answer: `Оцените совместимость ${material} с учётом рабочей жидкости, её концентрации, температуры, времени контакта и способа очистки.` },
    { question: `Что следует проверить при установке этого изделия серии «${series}»?`, answer: "Проверьте торец трубки, резьбу, уплотнительный элемент и поверхность порта. Они должны быть чистыми и неповреждёнными; не допускайте чрезмерной затяжки." },
    { question: `Можно ли запросить 2D-чертёж модели ${model}?`, answer: "Да. Добавьте модель в список и укажите необходимость чертежа; инженеры подтвердят применимую версию." },
  ];
}
...
export function localizeHardTubeFittingDetailData<T extends DetailRecord>(
  ...
  const sourceSpecs = Array.isArray(english.specs) ? english.specs : [];
  const seriesId = String(sourceData.seriesId || english.seriesId || "standard-flat-bottom-fitting");
  const model = String(english.model || sourceData.model || "");
  const series = SERIES_NAMES[locale][seriesId] || SERIES_NAMES[locale]["standard-flat-bottom-fitting"];
  const sealing = SEALING_NAMES[locale][getSealKey(seriesId)];
  const tubeOd = getSpecValue(sourceSpecs, ["Tube OD"]);
  const thread = getSpecValue(sourceSpecs, ["Thread Size", "Thread"]);
  const material = getSpecValue(sourceSpecs, ["Body Material", "Housing Material", "Material"]);
  const pressure = getSpecValue(sourceSpecs, ["Rated Pressure", "Pressure Rating"]);
  const description = buildLocalizedNarrative(locale, { model, series, tubeOd, thread, material, sealing, pressure });
  const localizedSeries = titleCaseFirst(series);

  const localizedSpecs = sourceSpecs.map((item: { label: string; value: string }) => {
    let value = item.value;

    if (item.label === "Product Series") value = localizedSeries;
    if (item.label === "Sealing Method") value = titleCaseFirst(sealing);
    if (item.label === "Color") value = COLOR_NAMES[locale][item.value] || item.value;

    return {
      ...item,
      label: SPEC_LABELS[locale][item.label] || item.label,
      value,
    };
  });
  ...
  const localized = {
    ...english,
    __locale: locale,
    categoryLabel: locale === "es" ? "Racores" : locale === "fr" ? "Raccords" : locale === "ko" ? "피팅" : "Фитинги",
    productTypeName: localizedSeries,
    seriesName: localizedSeries,
    name: localizedSeries,
    title: model,
    imageAlt: `${model} ${localizedSeries} ${material}`,
    imageAltEn: `${model} ${localizedSeries} ${material}`,
    mainImageAlt: `${model} ${localizedSeries} ${material}`,
    description,
    advantages: [
      `${SPEC_LABELS[locale]["Tube OD"]}: ${tubeOd}`,
    ...
    commonApplications: buildApplications(locale),
    specs: localizedSpecs,
    specifications: localizedSpecs,
    faqs: buildFaqs(locale, model, tubeOd, thread, material, series),
    detailHref: `/${locale}/products/fittings/hard-tube-fittings/${sourceData.slug || english.slug}`,
    href: `/${locale}/products/fittings/hard-tube-fittings/${sourceData.slug || english.slug}`,
    selectionHref: `/${locale}/products/?category=fittings&productType=hard-tube-fittings`,
    bottomCtaTitle: locale === "es" ? "Racores para pedidos por volumen y aplicaciones personalizadas" : locale === "fr" ? "Raccords pour commandes en volume et applications sur mesure" : locale === "ko" ? "대량 주문 및 맞춤 응용을 위한 피팅" : "Фитинги для серийных заказов и нестандартных применений",
    bottomCtaDescription: locale === "es" ? "FOREACH ofrece selección, suministro por volumen y personalización de racores según el tipo de conexión, el tamaño del tubo, el material, el elemento de sellado, el montaje y las condiciones de trabajo." : locale === "fr" ? "FOREACH assure la sélection, la fourniture en volume et la personnalisation des raccords selon le type de port, le diamètre du tube, le matériau, l’élément d’étanchéité, le montage et les conditions d’utilisation." : locale === "ko" ? "FOREACH는 포트 형식, 튜브 규격, 재질, 밀봉 부품, 장착 구조 및 사용 조건에 따라 피팅 선정, 대량 공급 및 맞춤 제작을 지원합니다." : "FOREACH помогает с подбором, серийной поставкой и изготовлением фитингов по требованиям к типу порта, размеру трубки, материалу, уплотнению, монтажной конструкции и условиям эксплуатации.",
    bottomCtaButtonText: locale === "es" ? "Contactar" : locale === "fr" ? "Nous contacter" : locale === "ko" ? "문의하기" : "Связаться с нами",
    bottomCtaHref: `/${locale}/contact`,
    seo: {
      title: `${model} ${localizedSeries} | FOREACH`,
      description,
    },
  } as DetailRecord;
