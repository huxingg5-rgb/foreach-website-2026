...
section("五、14 条无竞品关系的正式 Q20");

const formalQ20WithoutReplacement =
  formalQ20Products.filter(
    (product) => {
      ...
      return !bridgedCodes.has(code);
    }
  );

line(
  `未配置竞品关系的正式 Q20：${formalQ20WithoutReplacement.length}`
);

for (
  const product of
    formalQ20WithoutReplacement
) {
  line(
    `- ${
      product.productCode ||
...
assert.strictEqual(
  formalQ20WithoutReplacement.length,
  14,
  "未配置竞品关系的正式 Q20 数量不是 14"
);
...
line("5. 14 条无竞品关系的正式 Q20 没有被伪造加入。");
line("6. 其他接头数据没有被本次桥接改变。");
line("7. Q40、Q60 没有生成虚假的替代关系。");
line("");
line("本次只读取，没有修改网站源码。");
