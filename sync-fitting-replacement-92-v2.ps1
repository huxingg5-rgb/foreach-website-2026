...
const targetRows = JSON.parse(fs.readFileSync(targetJsonPath, "utf8"));
...
function analyzeAndPrepare(filePath) {
  const source = fs.readFileSync(filePath, "utf8");
  const parsed = parseSource(filePath, source);
  const edits = [];
  const matches = [];
...
try {
  ...
  const secondCheck = replacementFiles.map(analyzeAndPrepare);
