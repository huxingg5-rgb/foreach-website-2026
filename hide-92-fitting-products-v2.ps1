...
$targets = foreach ($row in $rows) {
    ...
    # 型号允许为空。
    # 本次下架以商品编码 / 产品 ID 为主键精确匹配；
    # 只有数据对象本身没有编码时，才会使用型号兜底。
    [PSCustomObject]@{
...
$blankModelCount = @(
    $targets |
        Where-Object {
            [string]::IsNullOrWhiteSpace($_.model)
        }
).Count

Write-Host "清单商品编码：92 个；型号为空：$blankModelCount 条。" -ForegroundColor Yellow
Write-Host '型号为空不会影响按商品编码精确下架。' -ForegroundColor Yellow
Write-Host ''
