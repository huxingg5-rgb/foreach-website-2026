...
export function localizeTargetProductDetailData<T extends DetailRecord>(sourceData: T, locale: HardTubeTargetLocale, pathname = ""): T {
  ...
  const localized = {
    ...
    seo: { ...(english.seo || {}), title: `${model} ${productName} | Foreach Technology`, description },
...
export function getTargetProductMetadataCopy(segments: string[], locale: HardTubeTargetLocale) {
  ...
  if (isSelectionRoute) {
    ...
    return { title: `${selectionName} | Foreach Technology`, description, keywords: [selectionName, CATEGORY_NAMES[locale][inferCategory(kind)] || selectionName, "Foreach Technology"] };
  }
  const slug = String(segments.at(-1) || "Foreach Technology");
  ...
  const model = tubingTitle || slug
    ...
    .replace(/-EP-PS$/, "-EP/PS");
  const description = buildDescription(locale, model, productName);
  return { title: `${model} ${productName} | Foreach Technology`, description, keywords: [model, productName, CATEGORY_NAMES[locale][inferCategory(kind)], "Foreach Technology"] };
}
