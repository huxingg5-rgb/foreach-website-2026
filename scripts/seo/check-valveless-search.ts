...
if(process.argv.includes("--capture")) {
  ...
  for(const locale of valvelessForeignLocales) {
    for(const slug of ["rpl-p4","rpl-p635","rpl-p15","drpl-0109","drpl-0119"]) {
      const item=current[locale].find(item=>item.h?.endsWith(`/valveless-pumps/${slug}`));
  ...
  for (const [locale, items] of Object.entries(current)) {
    const products = items.filter(item => /\/products\/pumps\/valveless-pumps\/[^/]+\/?$/.test(String(item.h || item.href)));
    ...
    assert.ok(!items.some(item => /\/valveless-pumps\/drpl\/?$/.test(String(item.h || item.href))), `${locale}: no retired DRPL route`);
  }
  for (const item of current["zh-CN"].filter(item => /\/valveless-pumps\/[^/]+$/.test(String(item.h)))) {
