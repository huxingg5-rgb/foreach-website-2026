...
write(
  path.join(root, "app/products/tubing/_components/TubingDetailStaticPage.tsx"),
  shared
);
...
for (const slug of pages) {
  const page = `import TubingDetailStaticPage, { getTubingMetadata } from "../_components/TubingDetailStaticPage";

export const metadata = getTubingMetadata("${slug}");
...
`;
  ...
  write(path.join(root, "app/products/tubing", slug, "page.tsx"), page);
}

console.log("");
console.log("已创建 6 个固定静态管路详情页。");