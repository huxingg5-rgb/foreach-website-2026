...
$scriptPath = Join-Path $root "scripts\resources\audit-compatible-model-product-links.ts"
$reportPath = Join-Path $root "docs\resources\compatible-model-product-link-audit.md"

New-Item -ItemType Directory -Force -Path (Split-Path $scriptPath), (Split-Path $reportPath) | Out-Null
...
Write-Host ""
Write-Host "开始检查兼容型号与正式产品页面的关联……" -ForegroundColor Cyan
npx tsx scripts/resources/audit-compatible-model-product-links.ts

Write-Host ""
Write-Host "本步骤只生成审计报告，没有修改页面。" -ForegroundColor Green
