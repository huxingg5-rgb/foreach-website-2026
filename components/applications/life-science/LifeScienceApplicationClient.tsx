...
/* =========================================================
   LifeScienceApplicationClient.tsx
   恒永达官网｜生命科学应用领域交互组件
   ...
   说明：
   1. 样式 class 复用 IVD 应用页，保证应用领域页面风格一致
   2. 内容和数据独立维护在 data/applications/life-science/
   3. 支持 URL 参数 ?application=xxx 自动选中应用类型
   4. 面包屑使用 application-breadcrumb，不沿用 IVD 错误面包屑
========================================================= */
...
import type {
  ...
  LifeScienceApplicationType,
  LifeScienceFluidicModule,
} from "@/data/applications/life-science/life-science-application.types";

type LifeScienceApplicationClientProps = {
  data: LifeScienceApplicationPageData;
};
...
export default function LifeScienceApplicationClient({ data }: LifeScienceApplicationClientProps) {
  ...
  const activeApplication = useMemo<LifeScienceApplicationType>(() => {
    return (
  ...
  const activeModule = useMemo<LifeScienceFluidicModule>(() => {
    return (
  ...
  function handleApplicationChange(application: LifeScienceApplicationType) {
  ...
  function handleModuleChange(module: LifeScienceFluidicModule) {
    setActiveModuleKey(module.key);
    setActiveProductKey(module.products[0] ?? "");
  }

  return (
    <main className="ivd-page life-science-page">
      {/* =====================================================
          Banner
          说明：
          1. 主体样式复用 IVD
          2. 面包屑使用 application-breadcrumb，不使用 IVD 错误面包屑
      ===================================================== */}
...
{/* =====================================================
      ...
      <section className="ivd-section" id="applications">
        <div className="ivd-section-inner">
          ...
          <div className="ivd-instrument-tabs" role="tablist" aria-label="生命科学应用类型">
