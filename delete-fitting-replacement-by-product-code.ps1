...
#
# 处理范围：
# 1. all-compatible-products.generated.ts
# 2. compatible-models.generated.ts
#
# 删除规则：
# - 从原始 92 个无主图接头清单读取商品编码 / 产品 ID
# - 只按 productCode / productId / code / sku 精确匹配
# - 不使用型号兜底
# - 同一文件中同一商品编码必须唯一
# - 两个数据文件命中的商品编码集合必须完全一致
#
# 如果两个文件都已经没有目标编码：
# - 直接报告“已经清理”，不重复修改
#
  ...
  for (const statement of parsed.statements) {
    ...
    for (const declaration of statement.declarationList.declarations) {
      ...
      for (const element of arrayNode.elements) {
        ...
        const matchedCode = record.codeValues.find((value) =>
...
const preparedFiles = replacementFiles.map((filePath) =>
  analyzeAndPrepare(filePath, { allowZero: true })
);

const allMatches = preparedFiles.flatMap((item) => item.matches);

function getUniqueCodesForFile(item) {
  const codes = item.matches
    .map((match) => normalize(match.productCode))
    .filter(Boolean);
  ...
  if (codes.length !== uniqueCodes.size) {
    ...
    throw new Error(
      `${rel(item.filePath)} 中同一商品编码匹配了多条记录：` +
        duplicateCodes.join(", ") +
        "\n商品编码应当唯一，脚本已停止，没有修改文件。"
    );
  }
...
const codeSets = preparedFiles.map(getUniqueCodesForFile);
const firstCodeSet = codeSets[0];
const secondCodeSet = codeSets[1];

const onlyInFirst = [...firstCodeSet].filter(
  (code) => !secondCodeSet.has(code)
);
const onlyInSecond = [...secondCodeSet].filter(
  (code) => !firstCodeSet.has(code)
);

console.log("");
console.log("接头替代查询商品编码精确删除复查：");
console.log(`原始目标商品编码：${targetCodes.size} 个`);

for (const item of preparedFiles) {
  console.log(
    `- ${rel(item.filePath)}：命中 ${item.removedCount} 个唯一商品编码，修改 ${item.editedArrays} 个数组`
  );
}
...
if (
  ...
  throw new Error(
    "两个接头替代数据文件命中的商品编码集合不一致。\n" +
      `只在第一个文件：${onlyInFirst.join(", ") || "无"}\n` +
      `只在第二个文件：${onlyInSecond.join(", ") || "无"}\n` +
      "脚本已停止，没有修改文件。"
  );
}

const matchedCodes = firstCodeSet;

if (matchedCodes.size === 0) {
  console.log("");
  console.log("两个接头替代数据文件中已经没有这 92 个商品编码。");
  console.log("无需重复删除，本次没有修改任何文件。");
  process.exit(0);
}

if (allMatches.length !== matchedCodes.size * replacementFiles.length) {
  throw new Error(
    "删除记录数量与唯一商品编码数量不一致。\n" +
      `唯一商品编码：${matchedCodes.size} 个；记录：${allMatches.length} 条。\n` +
      "脚本已停止，没有修改文件。"
  );
}

console.log(
  `实际需要删除：${matchedCodes.size} 个商品、${allMatches.length} 条记录`
);
...
/*
 * 历史构建失败快照曾以 .ts / .tsx 文件保存到 audit-reports。
 * Next.js/TypeScript 会扫描这些文件，导致快照中的相对 import 无法解析。
 *
 * 构建前只处理审计目录：
 * - .ts  -> .ts.txt
 * - .tsx -> .tsx.txt
 *
 * 不会触碰 data、components、app 等正式源码目录。
 */
function quarantineAuditTypeScriptFiles(directory) {
...
const legacyAuditRoot = path.join(
  root,
  "audit-reports/fitting-replacement-direct-remove"
);

const quarantinedAuditFiles =
  quarantineAuditTypeScriptFiles(
    legacyAuditRoot
  );

if (quarantinedAuditFiles.length > 0) {
  console.log("");
  console.log(
    `已将 ${quarantinedAuditFiles.length} 个历史审计 .ts/.tsx 快照改名为 .txt，避免参与类型检查。`
  );
...
if (
  ...
  for (const item of preparedFiles) {
    /*
     * 诊断副本必须使用 .txt 后缀。
     * 如果继续保存为 .ts，Next.js 会再次把审计快照纳入类型检查。
     */
    fs.copyFileSync(
      item.filePath,
      path.join(
        failedStateDirectory,
        `${path.basename(item.filePath)}.txt`
      )
    );
  }
...
console.log("");
console.log("============================================");
console.log("接头替代查询商品编码删除完成");
console.log("============================================");
console.log(`实际删除商品：${matchedCodes.size} 个`);
...
Write-Host ''
Write-Host '准备按唯一商品编码清理接头替代查询……' -ForegroundColor Cyan
Write-Host '两个数据文件命中的商品编码必须完全一致。' -ForegroundColor Yellow
Write-Host '不会使用型号兜底，也不会修改产品中心。' -ForegroundColor Yellow
Write-Host ''
...
if ($nodeExitCode -ne 0) {
    throw '接头替代查询商品编码删除未完成。请把完整错误和 npm-build.log 发来。'
}
