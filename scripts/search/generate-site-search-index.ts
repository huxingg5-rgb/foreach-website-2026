...
async function main() {
  ...
  const finalItems = deduplicate([
    ...
    ...compatibleItems,
    ...datasheetItems,
  ].map(item => ({ ...item, href: normalizePipettingPath(getSyringeModelRedirect(item.href) || getPistonPumpRedirect(item.href) || item.href) })).flatMap(normalizeValvelessSearchItem));
