...
$targets = foreach ($row in $rows) {
    $code = ([string]$row.'商品编码').Trim()
    $model = ([string]$row.'型号').Trim()
    $status = ([string]$row.'状态').Trim()
    $reason = ([string]$row.'结论').Trim()

    if ([string]::IsNullOrWhiteSpace($code)) {
        throw "清单中存在空商品编码，脚本已停止。"
    }

    if ($status -ne 'active') {
        throw "商品 $code 当前审计状态不是 active，而是：$status"
    }
...
if ($uniqueCodes.Count -ne 7) {
    throw "商品编码去重后不是 7 个，而是 $($uniqueCodes.Count) 个。"
}
...
if (targetCodes.size !== 7) {
  throw new Error(
    `目标商品编码去重后不是 7 个，而是 ${targetCodes.size} 个。`
  );
}
...
function prepareQuickSelection() {
  ...
  if (
    ...
    throw new Error(
      "快插选型匹配数量异常。\n" +
      `预期：7 个商品；实际：${matchedCodes.size} 个商品、${matches.length} 条记录。\n` +
      "脚本已停止，没有修改文件。"
    );
  }
