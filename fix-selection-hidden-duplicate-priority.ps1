# ============================================================
# 修复产品中心合并数据时 hidden 状态被 active 重复记录覆盖
#
# 背景：
...
if (
  ...
  throw new Error(
    "ProductSelectionClient.tsx 当前没有 active 状态过滤，脚本已停止。"
  );
}
...
const newCallback = `(product, index, array) => {
   ...
   * ${START_MARKER}
   *
   * 同一 productId 可能同时存在于基础数据和分类数据中。
   * 分类数据标记为 hidden 时，必须让 hidden 版本赢得去重，
   * 否则基础数据中的 active 重复记录会让已下架产品重新显示。
   */
  const hiddenIndex = array.findIndex((item) => {
    return (
  ...
  return index === (hiddenIndex >= 0 ? hiddenIndex : firstIndex);
...
fs.writeFileSync(
  ...
  [
    "# 产品中心 hidden 状态优先去重修复",
    "",
    "- 已验证 92 个目标在分类文件中全部为 hidden",
    "- 只修改 ProductSelectionClient.tsx",
    "- 同一 productId 存在 hidden 时，优先保留 hidden",
    "- 不存在 hidden 时，仍保留第一条记录",
    "- 未添加全局 Helper",
    "- 未修改产品数据和接头替代查询",
    "",
  ].join("\n"),
...
Write-Host ''
Write-Host '准备修复产品中心 hidden 状态被重复 active 数据覆盖的问题……' -ForegroundColor Cyan
Write-Host '先验证 92 个分类记录全部为 hidden。' -ForegroundColor Yellow
Write-Host '只修改 ProductSelectionClient.tsx 的 selectionProducts 去重规则。' -ForegroundColor Yellow
Write-Host ''
...
if ($nodeExitCode -ne 0) {
    throw "产品中心 hidden 优先去重修复未完成，请把完整错误和 npm-build.log 发来。"
}
