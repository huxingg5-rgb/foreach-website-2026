...
// ========== 核心计算：逐行流量求系统压降 ==========
function compute_system_with_row_flows(rows, fallback_flow_mlmin, fluid_type, temperature,
                                       user_rho, user_mu, user_nu, orifice_shape, cv_model) {
  const row_results = rows.map(row => {
    const parsed_flow = parseFloat(row.flow);
    const row_flow = Number.isFinite(parsed_flow) && parsed_flow > 0
      ? parsed_flow
      : fallback_flow_mlmin;
    return compute_system_with_flow(
      [row], row_flow, fluid_type, temperature,
      user_rho, user_mu, user_nu, orifice_shape, cv_model
    );
  });

  const total_dPy = row_results.reduce((sum, result) => sum + result.total_dPy, 0.0);
  const total_dPj = row_results.reduce((sum, result) => sum + result.total_dPj, 0.0);
  return {
    total_dPt: total_dPy + total_dPj,
    total_dPy,
    total_dPj,
    results: row_results.flatMap(result => result.results),
    Re_list: row_results.flatMap(result => result.Re_list)
  };
}
...
// ========== 生成逐行流量配置的 PQ 曲线 ==========
function generate_pq_curve_with_row_flows(rows, fluid_type, temperature,
                                          user_rho, user_mu, user_nu,
                                          orifice_shape, cv_model,
                                          Q_center, num_points = 200) {
  let Q_min = Q_center * Math.pow(10, -0.5);
  let Q_max = Q_center * Math.pow(10, 0.5);
  if (Q_min <= 0) Q_min = 0.001;
  ...
  for (let i = 0; i < num_points; i++) {
    const Q = Math.pow(10, logMin + (logMax - logMin) * i / (num_points - 1));
    const scale = Q_center > 0 ? Q / Q_center : 1.0;
    const scaled_rows = rows.map(row => {
      const parsed_flow = parseFloat(row.flow);
      const base_flow = Number.isFinite(parsed_flow) && parsed_flow > 0
        ? parsed_flow
        : Q_center;
      return { ...row, flow: base_flow * scale };
    });
    const result = compute_system_with_row_flows(
      scaled_rows, Q, fluid_type, temperature,
      user_rho, user_mu, user_nu, orifice_shape, cv_model
    );
    const valid_Re = result.Re_list.filter(r => r != null && r > 0);
    const Re_min = valid_Re.length > 0 ? Math.min(...valid_Re) : 0;
...
module.exports = {
  ...
  compute_system_with_flow, compute_system_with_row_flows,
  find_flow_by_pressure, generate_pq_curve, generate_pq_curve_with_row_flows,
