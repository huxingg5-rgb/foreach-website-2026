...
import { isFittingProductOffline } from "@/data/products/selection/fitting-offline-products.generated";
import { isQuickConnectProductOffline } from "@/data/products/selection/quick-connect-offline-products.generated";

const selectionProducts = /* QUICK_CONNECT_OFFLINE_FILTER_START */
([
...
].filter((product) => !isFittingProductOffline(product as unknown as Record<string, unknown>)).filter((product, index, array) => {
  return index === array.findIndex((item) => item.productId === product.productId);
})).filter((product) => !isQuickConnectProductOffline(product as unknown as Record<string, unknown>))
