...
export const productTypeIntroMap: Record<string, ProductTypeIntroContent> = {
  ...
  "pumps:diaphragm-pump": {
    ...
    paragraphs: [
      ...
      "选型时可根据**输送介质、目标流量、工作压力或真空度、自吸高度、电机类型、接液材质、接口形式及安装空间**进行筛选。隔膜泵包括 **DPL30 300 mL/min、DPL60 600 mL/min**，以及 **DPGL800 双头微型气液混合隔膜泵（单泵头空载气体流量为 6 L/min）** 等型号，可面向 IVD、实验室分析及医疗设备液路进行选型。",
  ...
  "pumps:diaphragm-pump": {
    ...
    es: {
      ...
      paragraphs: [
        ...
        "La selección puede filtrarse por **fluido bombeado, caudal objetivo, presión de trabajo o nivel de vacío, altura de autocebado, tipo de motor, materiales en contacto con el fluido, tipo de conexión y espacio de instalación disponible**. La gama de bombas de diafragma incluye la **DPL30 de 300 mL/min y la DPL60 de 600 mL/min**, además de la **bomba de diafragma miniatura de doble cabezal DPGL800 para gas y líquido, con un caudal de gas sin carga de 6 L/min por cabezal**, para circuitos fluídicos de equipos IVD, análisis de laboratorio y dispositivos médicos.",
    ...
    ko: {
      ...
      paragraphs: [
        ...
        "선정 시 **이송 유체, 목표 유량, 작동 압력 또는 진공도, 자흡 높이, 모터 유형, 접액부 재질, 포트 형식 및 설치 공간**을 기준으로 필터링할 수 있습니다. 다이어프램 펌프에는 **DPL30 300 mL/min 및 DPL60 600 mL/min**과 **헤드당 무부하 가스 유량이 6 L/min인 DPGL800 듀얼 헤드 소형 기액 혼합 다이어프램 펌프**가 있으며 IVD, 실험실 분석 및 의료기기 유로용으로 선정할 수 있습니다.",
